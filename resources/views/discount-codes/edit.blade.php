...
@section('content')
    <div class="max-w-7xl mx-auto py-6 px-2.5 sm:px-6 lg:px-8">
        <h2 class="text-2xl font-bold mb-6">Edit Discount Code</h2>

        <form action="{{ route('discount-codes.update', $discount_code) }}" method="post"
            class="space-y-4 bg-white shadow-md rounded-xl p-6">
            @csrf
            <div>
                <label class="block text-gray-700 font-medium mb-2">Code</label>
                <input type="text" name="code" value="{{ old('code', $discount_code->code) }}"
            ...
            <div>
                ...
                <input type="number" step="0.01" name="value" value="{{ old('value', $discount_code->value) }}"
                    class="w-full p-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500" required>
            ...
            <div>
                ...
                <select name="type"
                    class="w-full p-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500" required>
                    <option value="percent" {{ old('type', $discount_code->type) == 'percent' ? 'selected' : '' }}>Percent
                    </option>
                    <option value="fixed" {{ old('type', $discount_code->type) == 'fixed' ? 'selected' : '' }}>Fixed
                    </option>
                </select>
            ...
            <div>
                ...
                <input type="date" name="start_date"
                    value="{{ old('start_date', $discount_code->start_date ? \Carbon\Carbon::parse($discount_code->start_date)->format('Y-m-d') : '') }}"
            ...
            <div>
                ...
                <input type="date" name="end_date"
                    value="{{ old('end_date', $discount_code->end_date ? \Carbon\Carbon::parse($discount_code->end_date)->format('Y-m-d') : '') }}"
                    class="w-full p-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500">
