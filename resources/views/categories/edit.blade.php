...
<div class="max-w-7xl mx-auto py-6 px-2.5 sm:px-6 lg:px-8">
    ...
    <form action="{{ route('categories.update', $category) }}" method="POST" class="space-y-4 bg-white shadow-md rounded-xl p-6">
        @csrf
        <div>
            <label class="block text-gray-700 font-medium mb-2">Name</label>
            <input type="text" name="name" value="{{ old('name', $category->name) }}" class="w-full p-2 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500" required>
