...
@section('content')
    <div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        <h2 class="text-2xl font-bold mb-6">Add Purchase</h2>

        <form action="{{ route('purchases.store') }}" method="POST" class="space-y-4 bg-white shadow-md rounded-xl p-6">
            @csrf
            ...
            <div class="products-container space-y-4">
                ...
                <div class="product-row flex space-x-2">
                    <input type="text" name="products[0][product_name]" placeholder="Product Name"
                        class="flex-1 p-2 border border-gray-300 rounded-xl" required>
                    <input type="number" name="products[0][quantity]" value="1" min="1" placeholder="Quantity"
                        class="w-24 p-2 border border-gray-300 rounded-xl">
                    <input type="number" step="0.01" name="products[0][purchase_price]" value="0.00"
                        placeholder="Price" class="w-32 p-2 border border-gray-300 rounded-xl">
                    <button type="button" class="add-product bg-green-500 text-white px-3 rounded-lg">+</button>
                </div>
            </div>

            <div class="mt-4">
                <label class="block text-gray-700 font-medium mb-2">Total Amount</label>
                <input type="number" step="0.01" name="total_amount" value="0.00" readonly
            ...
            <div>
                ...
                <textarea name="notes" class="w-full p-2 border border-gray-300 rounded-xl">{{ old('notes') }}</textarea>
            </div>
